...
def lambda_handler(event, context):
    ...
    logger.info(f"Received event for bucket: {bucket_name}, key: {original_key}")
    ...
    try:
        ...
        logger.info(f"Downloading image from S3: {bucket_name}/{original_key}")
        try:
            response = s3.get_object(Bucket=bucket_name, Key=original_key)
        except ClientError as e:
            if e.response['Error']['Code'] == 'NoSuchKey':
                logger.error(f"Object not found in S3 bucket: {bucket_name}, key: {original_key}")
            elif e.response['Error']['Code'] == 'AccessDenied':
                logger.error(f"Access denied to S3 bucket: {bucket_name}, key: {original_key}")
            else:
        ...
        for size_name, size in sizes.items():
            ...
            logger.info(f"Uploading resized image to S3: {bucket_name}/{resized_key}")
